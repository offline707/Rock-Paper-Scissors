...
async function startGame(playerChoiceHTML){
    ...
    if(computerScore === 2 || playerScore === 2) return 0;
    ...
    function win(){
        ...
        <p style='color: #73E9BA;'>You won, Anon!</p>
        ` ;
    ...
    function loss(){
        ...
        <p style='color: red;'>HAHAHA You Lost!</p>
        ` ;
...
async function computerAnimation(computerChoice){
    ...
    for(let i=0; i<15; i++){
        await delay(15*i);
        computerImage.src = `src/${choice[(i+1)%3]}.png`
    }
    computerImage.src = `src/${computerChoice}.png`
}